...
size_t RedBlackTree::rankOf(int key) const {
    ...
    while (current != nullptr) {
        if (current->key < key) {
            // Count the current node and all nodes in its left subtree
            count += 1 + countNodes(current->left);
...
int RedBlackTree::select(size_t rank) const {
    ...
    while (current != nullptr) {
        size_t leftSize = countNodes(current->left);
        if (rank == leftSize) {
            // Found the node with the desired rank
            return current->key;
        } else if (rank < leftSize) {
            // Move to the left subtree
            current = current->left;
        } else {
            // Adjust rank and move to the right subtree
            rank -= leftSize + 1;
            current = current->right;
        }
    }

    // If rank exceeds the number of elements in the tree
    throw std::runtime_error("The red/black tree does not contain at least " + std::to_string(rank + 1) + " elements.");
}

size_t RedBlackTree::countNodes(Node* node) const {
    if (node == nullptr) {
        return 0;
    }
